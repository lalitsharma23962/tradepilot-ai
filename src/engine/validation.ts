...
export function evaluateWalkForwardValidation(folds:ValidationFold[],minOosTradesPerFold=10,minTotalOosTrades=30):ValidationSummary{
 ...
 if(totalOosTrades<minTotalOosTrades)return{status:'INSUFFICIENT_DATA',totalOosTrades,minOosTradesRequired:minTotalOosTrades,folds,reason:`Total OOS trades (${totalOosTrades}) below required minimum (${minTotalOosTrades}).`};
 const allPassed=folds.length>0&&folds.every(f=>f.oosTrades>=minOosTradesPerFold&&f.profitFactor>1.05&&f.status==='PASS');
 return{status:allPassed?'PRODUCTION_CANDIDATE':'REJECTED',totalOosTrades,minOosTradesRequired:minTotalOosTrades,folds,reason:allPassed?'All folds passed minimum trade and profitability gates.':'One or more folds failed minimum OOS trades, profitability, or status checks.'};
}
