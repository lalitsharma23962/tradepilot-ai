...
export interface SizingResult{requestedRiskPct:number;riskBudgetDollars:number;stopDistance:number;riskPerUnit:number;calculatedQuantity:number;maxNotionalQuantity:number;finalQuantity:number;bindingConstraint:string;effectiveNotional:number;effectiveRiskPct:number;}
...
export function calculatePositionSize(p:SizingParameters):SizingResult{
 ...
 if(stopDistance<=0||entryPrice<=0||accountEquity<=0)return{requestedRiskPct:riskPct,riskBudgetDollars:0,stopDistance,riskPerUnit:0,calculatedQuantity:0,maxNotionalQuantity:0,finalQuantity:0,bindingConstraint:'INVALID_INPUT',effectiveNotional:0,effectiveRiskPct:0};
 ...
 return{requestedRiskPct:riskPct,riskBudgetDollars,stopDistance,riskPerUnit:stopDistance,calculatedQuantity,maxNotionalQuantity:Math.min(maxNotionalQuantity,maxLeverageQuantity),finalQuantity,bindingConstraint,effectiveNotional,effectiveRiskPct};
}
