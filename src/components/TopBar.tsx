...
export function TopBar({ page }: { page: Page }) {
  const { account, conn, refresh } = useApp();
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      if (account?.started_at) {
        setUptime(Math.round((Date.now() - new Date(account.started_at).getTime()) / 1000));
      }
    }, 1000);
  ...
  const handleRestart = async () => {
    ...
    if (!res.ok) setMsg(res.error ?? 'Failed to restart');
    ...
    <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-slate-800 bg-slate-900/80 px-6 backdrop-blur-md">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-1.5 text-xs text-slate-500">
          ...
          <span className="tabular-nums">{fmtMoney(account?.equity)}</span>
        </div>
