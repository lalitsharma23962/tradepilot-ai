...
/** V21: strict production validation for high-RR paper strategy. */
...
function summarize(id:string, returns:number[], initial:number):StrategyResult {
  ...
  return {id,name:'Production Regime Breakout v21',trades:returns.length,wins:wins.length,losses:losses.length,winRate:wr,profitFactor:pf,netPnl:equity-initial,returnPct:ret,maxDrawdownPct:dd,avgTrade:avg,score:(ret+Math.min(pf,5)*2.5+sh*2+so*.75+wr/25-dd*.8)*Math.min(1,returns.length/30),tradeReturnsPct:returns,sharpe:sh,sortino:so,calmar:dd?ret/dd:0,expectancy:avg,turnoverPct:returns.reduce((a,b)=>a+Math.abs(b),0)};
}
  ...
  for(let i=Math.max(start,LOOKBACK);i<end;i++){
    ...
    if(!open){
      ...
      if(signal.action!=='WAIT'&&signal.riskReward>=LOW_RR){
        ...
        // The stop must be wide enough to make the declared RR meaningful after costs.
        // This prevents tiny structural stops from turning fees/slippage into a hidden
        // multiple of 1R and was a major source of the v16 PF collapse.
...
export async function runValidation(symbol='BTCUSDT',interval='1h',cfg:Partial<BacktestConfig>={},_selectedStrategyId?:string):Promise<ValidationReport>{
  const config={initialCapital:10000,feeBps:10,slippageBps:2,riskPerTradePct:.25,maxPositionPct:20,leverage:10,stopAtr:1.15,rewardRisk:10,maxBarsInTrade:interval==='15m'?480:interval==='4h'?180:240,...cfg};
  ...
  if(!allThree){const passed=folds.filter(foldPass).length;reasons.push(`Production Regime Breakout v21 did not pass all 3 pre-OOS stability folds (${passed}/3 passed; minimum ${MIN_FOLD_TRADES} trades/fold, PF >= ${MIN_PF}, positive return, DD <= ${MAX_DD}%).`);}
