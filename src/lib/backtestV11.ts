...
// v35 needs enough preceding history for genuinely non-overlapping capacity
// episodes before a fold's first scored decision. This is warm-up/context only:
// trades before WARMUP_BARS are never included in a fold's performance.
const CAPACITY_CONTEXT=LOOKBACK+MAX_CAPACITY_HORIZON*(MIN_INDEPENDENT_SAMPLES+1)+300;
const WARMUP_BARS=CAPACITY_CONTEXT;
...
export async function runValidation(symbol='BTCUSDT',interval='5m',cfg:Partial<BacktestConfig>={},_selectedStrategyId?:string):Promise<ValidationReport>{
 ...
 const candles=fetched.slice(-MAX_HISTORY_BARS),n=candles.length;
 if(n<=WARMUP_BARS+FOLDS)throw new Error(`Need more history for v35 independent-capacity warm-up: ${WARMUP_BARS.toLocaleString()} candles are reserved before scored validation begins.`);
 // The first WARMUP_BARS are context only. The pre-OOS folds begin after that
 // warm-up so fold 1 has the same capacity-history requirement as folds 2/3.
 // This does not add any trades to the scored sample and does not use future
 // bars: every capacity sample still ends before its decision bar.
 const scoredBars=n-WARMUP_BARS,preBars=Math.floor(scoredBars*PRE_OOS),preStart=WARMUP_BARS,preEnd=preStart+preBars,foldSize=Math.floor(preBars/FOLDS),funnel=newFunnelCounters(),familyReturns:FamilyTaggedReturn[]=[],foldDiagnostics:Record<string,FoldDiagnostic[]>={production:[]},folds:StrategyResult[]=[];
