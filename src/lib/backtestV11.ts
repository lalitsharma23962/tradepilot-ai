...
/** V26: strict production validation for the multi-family high-RR paper strategy. */
...
export async function runValidation(symbol='BTCUSDT',interval='1h',cfg:Partial<BacktestConfig>={},_selectedStrategyId?:string):Promise<ValidationReport>{
  ...
  if(!allThree){
    ...
    if(folds.every(f=>f.trades===0)) reasons.push(`Production Regime Breakout v26 generated zero trades in all ${FOLDS} pre-OOS folds. The strategy is not eligible for paper trading.`);
    else reasons.push(`Production Regime Breakout v26 did not pass all ${FOLDS} pre-OOS stability folds (${passed}/${FOLDS} passed). ${folds.map(foldReason).join(' | ')}`);
  }
  ...
  return{symbol,interval,candles:n,dataQuality:{startTime:candles[0].openTime,endTime:candles.at(-1)!.openTime,durationDays:(candles.at(-1)!.openTime-candles[0].openTime)/864e5,expectedIntervalMinutes:step/60000,gaps:candles.slice(1).filter((x,i)=>x.openTime-candles[i].openTime!==step).length,duplicateTimestamps:n-new Set(candles.map(x=>x.openTime)).size},costs:{feeBps:config.feeBps,slippageBps:config.slippageBps,roundTripPct:2*(config.feeBps+config.slippageBps)/10000},strategies,walkForward:{trainBars:foldSize,validationBars:foldSize,testBars:n-pre,selectedStrategy:allThree?'Production Regime Breakout v26':'No eligible strategy',validation:allThree?validation:null,test},foldDiagnostics,monteCarlo:mc,gate,generatedAt:new Date().toISOString(),research:{asOf:new Date().toISOString(),dataWindowBars:n,selectionMethod:'Strict production strategy v26; 3 non-overlapping pre-OOS folds; 10R/15R targets with execution-aligned structural risk; structurally oversized stops are rejected rather than compressed; four independent setup families (trend continuation, fresh breakout, breakout retest/pullback, compression expansion) share regime/momentum/cost/extension guards; no pre-target profit lock/trailing exits; no same-bar re-entry; next-candle gap protection; all 3 folds required; untouched 30% OOS; costs included; no manual OOS tuning.',coverage:['10R target for score 90-95','15R target for ultra-quality score 96+','structural stop integrity: reject oversized stops','cost-aware minimum risk distance','ATR/structural stop floor','trend-efficiency and directional-consistency filter','fresh range breakouts','breakout retests and EMA pullbacks','compression-to-expansion continuation','trend continuation','momentum-confirmed setup families','controlled extension filter','next-candle gap protection','target-integrity: no profit lock before 10R/15R','no same-candle replacement entry','5m high-RR timeout extended to 1440 bars','strict 3-fold stability gate','untouched 30% OOS','5,000-run Monte Carlo loss test']}};
}