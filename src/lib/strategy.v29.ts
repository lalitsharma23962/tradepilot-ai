...
/** v29: adaptive regime strategy. Trend entries require an actual pullback/reclaim; range entries use an RSI/Bollinger reversal trigger. The validation gate remains external and unchanged. */
export function evaluateProductionStrategy(input:number[]|MarketBar[],config:Partial<StrategyConfig>={}):StrategySignal{
 ...
 const breakoutVolume=avgVolume<=0||volumeRatio>=.95;
 ...
 // v28's standalone continuation family was the dominant losing family (46 trades, PF 0.59 in the latest 60k-candle run). v29 only accepts continuation when it contains a real recent pullback and a tight reclaim, avoiding late trend chasing.
 const continuationLongLocal=up&&e9>e20&&s24>0&&eff24>=.16&&eff48>=.10&&longConsistency>=.50&&recentPullbackLong&&reclaimLong&&Math.abs(entry-e20)<=a*1.0,continuationShortLocal=down&&e9<e20&&s24<0&&eff24>=.16&&eff48>=.10&&shortConsistency>=.50&&recentPullbackShort&&reclaimShort&&Math.abs(entry-e20)<=a*1.0;
 ...
 const rangeRegime=sep<=.035&&eff24<=.28&&eff48<=.22&&expansion<=1.08;
 const reversionLongLocal=rangeRegime&&prevRsi<=30&&rrsi>prevRsi&&rrsi>=30&&entry>prevBar.close&&barLong&&entry<=lower20+a*.25;
 const reversionShortLocal=rangeRegime&&prevRsi>=70&&rrsi<prevRsi&&rrsi<=70&&entry<prevBar.close&&barShort&&entry>=upper20-a*.25;
 const costAware=vol>=Math.max(.00025,cost*.45)&&vol<=.05,notExtended=Math.abs(entry-e20)<=a*2.5;
 const localLong=breakoutLongLocal||pullbackLongLocal||continuationLongLocal||compressionLongLocal||reversionLongLocal,localShort=breakoutShortLocal||pullbackShortLocal||continuationShortLocal||compressionShortLocal||reversionShortLocal;
 const breakoutLong=breakoutLongLocal&&breakoutVolume,breakoutShort=breakoutShortLocal&&breakoutVolume;
 ...
 const reversionLong=reversionLongLocal&&!hShort,reversionShort=reversionShortLocal&&!hLong;
 const familyLong=breakoutLong||pullbackLong||continuationLong||compressionLong||reversionLong,familyShort=breakoutShort||pullbackShort||continuationShort||compressionShort||reversionShort;
 const trendLongScore=(up?20:0)+(hLong?15:0)+(e9>e20?7:0)+(momentumLong?20:0)+( (pullbackLong||continuationLong||breakoutLong||compressionLong)?15:0)+(pullbackLong?4:0)+(breakoutLong?4:0)+(continuationLong?4:0)+(compressionLong?4:0)+(rrsi>=45&&rrsi<=75?7:0)+(costAware?4:0)+(notExtended?3:0);
 const trendShortScore=(down?20:0)+(hShort?15:0)+(e9<e20?7:0)+(momentumShort?20:0)+((pullbackShort||continuationShort||breakoutShort||compressionShort)?15:0)+(pullbackShort?4:0)+(breakoutShort?4:0)+(continuationShort?4:0)+(compressionShort?4:0)+(rrsi>=25&&rrsi<=55?7:0)+(costAware?4:0)+(notExtended?3:0);
 const reversionLongScore=(rangeRegime?20:0)+(reversionLong?35:0)+(prevRsi<=25?15:0)+(rrsi>=30&&rrsi<=42?10:0)+(barLong?8:0)+(costAware?7:0)+(Math.abs(entry-mid20)<=a*1.5?5:0);
 const reversionShortScore=(rangeRegime?20:0)+(reversionShort?35:0)+(prevRsi>=75?15:0)+(rrsi<=70&&rrsi>=58?10:0)+(barShort?8:0)+(costAware?7:0)+(Math.abs(entry-mid20)<=a*1.5?5:0);
 const longScore=reversionLong?reversionLongScore:trendLongScore,shortScore=reversionShort?reversionShortScore:trendShortScore;
 if(cfg.funnel){
  if(continuationLongLocal||continuationShortLocal)cfg.funnel.familyCandidatesTrend++;
  if((breakoutLongLocal&&breakoutVolume)||(breakoutShortLocal&&breakoutVolume))cfg.funnel.familyCandidatesBreakout++;
  ...
  const anyHtf=hLong||hShort;
  if(!familyLong&&!familyShort){if(!localLong&&!localShort)cfg.funnel.noLocalPattern++;else if(!(momentumLong||momentumShort)&&!(reversionLongLocal||reversionShortLocal))cfg.funnel.rejectedMomentum++;else if(!anyHtf&&!(reversionLongLocal||reversionShortLocal))cfg.funnel.rejectedHtf++;else cfg.funnel.rejectedScore++;}
 }
 ...
 if(!side)return wait(entry,['No setup family passed the local trigger, regime, momentum and cost gate'],score);
 ...
 const stopLoss=side==='LONG'?entry-risk:entry+risk,takeProfit=side==='LONG'?entry+targetDistance:entry-targetDistance,family=reversionLong||reversionShort?'retest':breakoutLong||breakoutShort?'breakout':continuationLong||continuationShort?'trend':'compression',familyLabel=reversionLong||reversionShort?'Range RSI/Bollinger reversal':family==='breakout'?'Fresh breakout':family==='retest'?'EMA20 pullback/reclaim':family==='compression'?'Compression expansion':'Trend continuation';
 if(cfg.funnel)cfg.funnel.tradesOpened++;
 return{action:side,score:Math.round(clamp(score,0,100)),confidence:Math.round(clamp(score,0,100)),strategy:'Production Regime Breakout v29',entry,stopLoss,takeProfit,riskReward:rr,family,reasons:[familyLabel,side==='LONG'?'Bullish local regime':'Bearish local regime','Multi-horizon confirmation','Real OHLC ATR structural stop','Cost-aware risk distance',`Target ${rr.toFixed(1)}R`,`Score ${Math.round(score)}/100`]};
}
