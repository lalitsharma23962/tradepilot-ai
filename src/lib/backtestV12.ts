...
const PRE_OOS=TRADING_CONFIG.preOosFraction;
const FOLDS=TRADING_CONFIG.folds;
const LOOKBACK=TRADING_CONFIG.lookback;
const MIN_FOLD_TRADES=TRADING_CONFIG.minFoldTrades;
const MIN_TEST_TRADES=TRADING_CONFIG.minTestTrades;
const MIN_PF=TRADING_CONFIG.minProfitFactor;
const MAX_DD=TRADING_CONFIG.maxDrawdownPct;
const MAX_MC_LOSS=TRADING_CONFIG.maxMonteCarloLossProbability;
const MIN_SCORE=TRADING_CONFIG.minScore;
const HISTORY_TARGET:Record<string,number>={'5m':100000,'15m':100000,'30m':80000,'1h':50000,'2h':40000,'4h':19000,'1d':2500};

const intervalMs=(x:string)=>{const m=x.match(/^(\d+)([mhd])$/i);if(!m)throw new Error(`Unsupported interval: ${x}`);const n=+m[1],u=m[2].toLowerCase();return n*(u==='m'?60000:u==='h'?3600000:86400000);};
const mean=(a:number[])=>a.length?a.reduce((x,y)=>x+y,0)/a.length:0;

function summarize(id:string,rs:number[],initial:number):StrategyResult{
 const w=rs.filter(x=>x>0),l=rs.filter(x=>x<0),gp=w.reduce((a,b)=>a+b,0),gl=Math.abs(l.reduce((a,b)=>a+b,0)),pf=gl?gp/gl:0;
 let eq=initial,peak=initial,dd=0;
 for(const r of rs){eq*=1+r/100;peak=Math.max(peak,eq);dd=Math.max(dd,(peak-eq)/peak*100);}
 const ret=(eq/initial-1)*100,wr=rs.length?w.length/rs.length*100:0,avg=mean(rs),v=mean(rs.map(x=>(x-avg)**2)),sh=rs.length?Math.sqrt(rs.length)*avg/(Math.sqrt(v)||1):0;
 return{id,name:'Trend Pullback v39',trades:rs.length,wins:w.length,losses:l.length,winRate:wr,profitFactor:pf,netPnl:eq-initial,returnPct:ret,maxDrawdownPct:dd,avgTrade:avg,score:(ret+Math.min(pf,5)*2.5+sh*2+wr/25-dd*.8)*Math.min(1,rs.length/30),tradeReturnsPct:rs,sharpe:sh,sortino:sh,calmar:dd?ret/dd:0,expectancy:avg,turnoverPct:rs.reduce((a,b)=>a+Math.abs(b),0)};
}

function familySummary(rs:number[]):FamilyPerformance{
 const w=rs.filter(x=>x>0),l=rs.filter(x=>x<0),gp=w.reduce((a,b)=>a+b,0),gl=Math.abs(l.reduce((a,b)=>a+b,0)),pf=gl?gp/gl:(gp>0?Infinity:0);
 return{trades:rs.length,wins:w.length,winRate:rs.length?w.length/rs.length*100:0,profitFactor:Number.isFinite(pf)?pf:0,returnPct:rs.reduce((a,b)=>a+b,0),avgTrade:mean(rs)};
}

function monte(rs:number[],runs=TRADING_CONFIG.monteCarloRuns){
 if(!rs.length)return{simulations:runs,probabilityOfLoss:0,medianReturnPct:0,p05ReturnPct:0,p95MaxDrawdownPct:0};
 let seed=0x7a11>>>0;const rnd=()=>{seed=(1664525*seed+1013904223)>>>0;return seed/4294967296;};
 const finals:number[]=[],dds:number[]=[];
 for(let k=0;k<runs;k++){let e=1,p=1,d=0;for(let i=0;i<rs.length;i++){e*=1+rs[Math.floor(rnd()*rs.length)]/100;p=Math.max(p,e);d=Math.max(d,(p-e)/p*100);}finals.push((e-1)*100);dds.push(d);}
...
interface Tagged{pct:number;family:string;}
...
function simulate(c:Candle[],cfg:BacktestConfig,start:number,end:number,capacityContext:number,funnel?:FunnelCounters,familyReturns?:Tagged[]):StrategyResult{
 ...
 for(let i=Math.max(start,Math.min(LOOKBACK,220));i<end;i++){
  ...
  if(open){
   ...
   if(open.side===1?b.low<=open.stop:b.high>=open.stop){finish(open,open.stop);closed=true;}
    ...
    while(open&&open.targetIndex<DEFAULT_TARGETS.length){
     const tier=DEFAULT_TARGETS[open.targetIndex],target=open.entry+open.side*open.risk*tier.multipleR,hit=open.side===1?b.high>=target:b.low<=target;if(!hit)break;
     const q=open.targetIndex===DEFAULT_TARGETS.length-1?open.remainingQty:Math.min(open.remainingQty,open.initialQty*tier.allocationPct),exit=target*(1-open.side*slip),gross=open.side*(exit-open.entry)*q,fees=(Math.abs(open.entry*q)+Math.abs(exit*q))*fee;
     open.realizedPnl+=gross;open.realizedFees+=fees;open.remainingQty-=q;open.targetIndex++;
     if(open.targetIndex===1)open.stop=open.entry;else if(open.targetIndex===2)open.stop=open.entry+open.side*open.risk*.5;
    ...
    if(open){open.stop=runnerProtectedStop(open.side,open.entry,open.entry+open.side*open.risk*2,open.stop,b.high,b.low);if(open.bars>=cfg.maxBarsInTrade){finish(open,b.close);closed=true;}}
   }
  }
  if(!open&&!closed&&i<end-1){
   ...
   if(signal.action!=='WAIT'){
    ...
    if(sizing.finalQuantity>0&&Number.isFinite(risk)&&risk>0&&Number.isFinite(entry)&&sizing.effectiveNotional>=TRADING_CONFIG.minNotionalUsd){open={side,entry,stop,risk,initialQty:sizing.finalQuantity,remainingQty:sizing.finalQuantity,bars:0,family:signal.family,targetIndex:0,realizedPnl:0,realizedFees:0};if(funnel)funnel.tradesOpened++;}
   }
  }
 }
...
export async function runValidation(symbol='BTCUSDT',interval='5m',cfg:Partial<BacktestConfig>={},_selectedStrategyId?:string):Promise<ValidationReport>{
 const historyTarget=HISTORY_TARGET[interval]??50000,horizon=TRADING_CONFIG.maxBarsInTrade[interval]??240;
 const config={initialCapital:TRADING_CONFIG.paperStartingCapital,feeBps:TRADING_CONFIG.feeBps,slippageBps:TRADING_CONFIG.slippageBps,riskPerTradePct:TRADING_CONFIG.riskPerTradePct,maxPositionPct:TRADING_CONFIG.maxAllocationPct,leverage:TRADING_CONFIG.maxLeverage,stopAtr:TRADING_CONFIG.atrStopMultiple,maxBarsInTrade:horizon,...cfg};
 const capacityContext=Math.max(220,TRADING_CONFIG.capacitySamples*horizon+horizon+32);
 const fetched=await fetchHistoricalCandles(symbol,interval,historyTarget);
 if(fetched.length<historyTarget)throw new Error(`Need ${historyTarget.toLocaleString()} completed ${interval} candles; received ${fetched.length.toLocaleString()}.`);
 const candles=fetched.slice(-historyTarget),n=candles.length;
 if(n<=capacityContext+FOLDS*MIN_FOLD_TRADES)throw new Error(`Need more history for v39 validation: ${capacityContext.toLocaleString()} candles are reserved for causal capacity context.`);
 ...
 if(!allThree&&!insufficient){const failed=foldDiagnostics.production.filter(f=>!f.passed).map(f=>`fold ${f.fold} (trades ${f.trades}, PF ${f.profitFactor.toFixed(2)}, return ${f.returnPct.toFixed(2)}%, DD ${f.maxDrawdownPct.toFixed(2)}%)`);reasons.push(`Trend Pullback v39 failed chronological pre-OOS stability gates: ${failed.join('; ')}.`);}
