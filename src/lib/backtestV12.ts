...
function summarize(id:string,rs:number[],initial:number):StrategyResult{
 ...
 return{id,name:'Production Regime Breakout v38',trades:rs.length,wins:w.length,losses:l.length,winRate:wr,profitFactor:pf,netPnl:eq-initial,returnPct:ret,maxDrawdownPct:dd,avgTrade:avg,score:(ret+Math.min(pf,5)*2.5+sh*2+wr/25-dd*.8)*Math.min(1,rs.length/30),tradeReturnsPct:rs,sharpe:sh,sortino:sh,calmar:dd?ret/dd:0,expectancy:avg,turnoverPct:rs.reduce((a,b)=>a+Math.abs(b),0)};
}
