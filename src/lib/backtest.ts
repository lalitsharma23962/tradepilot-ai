...
export const DEFAULT_BACKTEST_CONFIG:BacktestConfig={initialCapital:10000,feeBps:10,slippageBps:2,riskPerTradePct:0.35,maxPositionPct:20,leverage:10,stopAtr:1.0,rewardRisk:4.0,maxBarsInTrade:48};
export const STRATEGIES=[
 {id:'production',name:'Production Confluence v4'},
...
function summarize(id:string,pnls:number[],initialCapital:number,maxDd:number):StrategyResult{
 ...
 return{id,name:STRATEGIES.find(s=>s.id===id)?.name??id,trades:pnls.length,wins,losses,winRate,profitFactor:pf,netPnl,returnPct,maxDrawdownPct:maxDd,avgTrade,score,tradeReturnsPct:pnls.map(p=>initialCapital?Math.max(-99,p/initialCapital*100):0)};
}
...
function simulateLegacyRange(cs:Candle[],id:string,cfg:BacktestConfig,startIndex=120,endIndex=cs.length):StrategyResult{
 ...
 let equity=cfg.initialCapital,peak=equity,maxDd=0;const pnls:number[]=[];
 ...
 for(let i=start;i<end;i++){
  const window=cs.slice(Math.max(0,i-INDICATOR_LOOKBACK+1),i+1),c=cs[i];
  if(open){
   open.bars++;const hitStop=open.side===1?c.low<=open.stop:c.high>=open.stop,hitTarget=open.side===1?c.high>=open.target:c.low<=open.target;
   if(hitStop||hitTarget||open.bars>=cfg.maxBarsInTrade){const raw=hitStop?open.stop:hitTarget?open.target:c.close;const pnl=closePnl(raw,open,fee,slip);pnls.push(pnl);equity+=pnl;open=null;}
  }
  if(!open){
   const side=legacySignal(id,window);
   if(side){
    const entry=c.close*(1+side*slip),a=atr(window),riskDistance=Math.max(a*cfg.stopAtr,entry*0.002),stop=entry-side*riskDistance,target=entry+side*riskDistance*cfg.rewardRisk,riskBudget=Math.max(0,equity)*cfg.riskPerTradePct/100;
    const riskQty=riskBudget/riskDistance,maxNotional=Math.max(0,equity)*cfg.maxPositionPct/100*Math.max(1,cfg.leverage),qty=Math.max(0,Math.min(riskQty,maxNotional/entry));
 ...
 if(open){const pnl=closePnl(cs[end-1].close,open,fee,slip);pnls.push(pnl);equity+=pnl;}
 return summarize(id,pnls,cfg.initialCapital,maxDd);
}
...
function simulateProductionRange(cs:Candle[],cfg:BacktestConfig,startIndex=120,endIndex=cs.length):StrategyResult{
 ...
 for(let i=start;i<end;i++){
  ...
  if(!open){
   ...
   const sig=evaluateStrategy(closes,{minScore:75,minRiskReward:4,maxRiskReward:8,atrStopMultiple:1,lookback:180});
   if(sig.action!=='WAIT'&&Number.isFinite(sig.entry)&&Number.isFinite(sig.stopLoss)&&Number.isFinite(sig.takeProfit)){
    const side=sig.action==='LONG'?1:-1,stopDistance=Math.abs(sig.entry-sig.stopLoss),targetDistance=Math.abs(sig.takeProfit-sig.entry);
    if(stopDistance>0&&targetDistance>0){
     const entry=c.open*(1+side*slip),stop=entry-side*stopDistance,target=entry+side*targetDistance;
     const riskBudget=Math.max(0,equity)*cfg.riskPerTradePct/100,riskQty=riskBudget/stopDistance,maxNotional=Math.max(0,equity)*cfg.maxPositionPct/100*Math.max(1,cfg.leverage),qty=Math.max(0,Math.min(riskQty,maxNotional/entry));
     if(qty>0)open={side,entry,stop,target,qty,bars:0};
    }
   }
  }
 ...
 if(open){const pnl=closePnl(cs[end-1].close,open,fee,slip);pnls.push(pnl);equity+=pnl;}
 return summarize('production',pnls,cfg.initialCapital,maxDd);
}
...
export async function fetchHistoricalCandles(symbol='BTCUSDT',interval='5m',limit=20000):Promise<Candle[]>{
 ...
 while(rows.length<target){
  const batchLimit=Math.min(1000,target-rows.length),url=`https://data-api.binance.vision/api/v3/klines?symbol=${encodeURIComponent(symbol)}&interval=${encodeURIComponent(interval)}&startTime=${cursor}&limit=${batchLimit}`;
  const response=await fetch(url,{headers:{Accept:'application/json'}});if(!response.ok)throw new Error(`Historical market data request failed (${response.status}).`);
  const batch=await response.json() as unknown[][];if(!batch.length)break;rows.push(...batch);const lastOpen=Number(batch[batch.length-1]?.[0]);if(!Number.isFinite(lastOpen)||lastOpen<cursor)break;cursor=lastOpen+intervalMs;if(batch.length<batchLimit)break;await new Promise(resolve=>setTimeout(resolve,60));
 }
...
function buildQuality(candles:Candle[],interval:string){
 const expected=intervalToMs(interval),gaps=candles.slice(1).filter((c,i)=>c.openTime-candles[i].openTime!==expected).length,duplicates=candles.length-new Set(candles.map(c=>c.openTime)).size;
 return{startTime:candles[0]?.openTime??0,endTime:candles[candles.length-1]?.openTime??0,durationDays:candles.length?(candles[candles.length-1].openTime-candles[0].openTime)/86400000:0,expectedIntervalMinutes:expected/60000,gaps,duplicateTimestamps:duplicates};
}

export async function runValidation(symbol='BTCUSDT',interval='5m',cfg:Partial<BacktestConfig>={}):Promise<ValidationReport>{
 const config={...DEFAULT_BACKTEST_CONFIG,...cfg,maxPositionPct:clamp(cfg.maxPositionPct??20,1,20),leverage:clamp(cfg.leverage??10,1,10)};
 ...
 const fullResults=STRATEGIES.map(s=>simulateRange(candles,s.id,config,120,candles.length)).sort((a,b)=>b.score-a.score),mc=monteCarlo(testResult.tradeReturnsPct),reasons:string[]=[];
