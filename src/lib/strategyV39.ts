...
export function evaluateV39(input: number[] | MarketBar[], config: Partial<StrategyConfig> = {}): StrategySignalV39 {
  ...
  if (Array.isArray(input) && input.length > 0) {
    if (typeof input[0] === 'number') {
      bars = (input as number[]).map((val) => ({ open: val, high: val, low: val, close: val, volume: 0 }));
  ...
  for (let i = 1; i < bars.length; i++) {
    const current = bars[i], prev = bars[i - 1];
    trValues.push(Math.max(current.high - current.low, Math.abs(current.high - prev.close), Math.abs(current.low - prev.close)));
  }
  ...
  const stopDistance = atr * atrMultStop, tpDistance = atr * atrMultTp;
  ...
  const targets: TargetLadderStep[] = [
    { r: 1, fraction: 0.5, price: action === 'LONG' ? entry + stopDistance : entry - stopDistance, moveStopToBreakeven: true },
    { r: atrMultTp / atrMultStop, fraction: 0.5, price: takeProfit, moveStopToBreakeven: false },
  ];
  return { action, family: 'TrendFollow', strategy: 'V39_ATR_Breakout', entry, stopLoss, takeProfit, riskReward, score: Math.min(100, Math.round((riskReward / 2) * 80)), targets, finalTargetR: atrMultTp / atrMultStop, reasons };
}
