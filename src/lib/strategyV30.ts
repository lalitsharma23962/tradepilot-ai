...
/** v30: score-first adaptive regime strategy. The validation gate remains strict; this revision removes redundant AND-gates from the signal families while keeping the strongest evidence as score requirements. */
export function evaluateProductionStrategy(input:number[]|MarketBar[],config:Partial<StrategyConfig>={}):StrategySignal{
 ...
 const reclaimLong=entry>e20&&entry>=prevBar.close&&barLong&&bodyRatio>=.22&&closeLocation>=.55,reclaimShort=entry<e20&&entry<=prevBar.close&&barShort&&bodyRatio>=.22&&closeLocation<=.45;
 const trendPullbackLong=up&&hLong&&recentPullbackLong&&reclaimLong,trendPullbackShort=down&&hShort&&recentPullbackShort&&reclaimShort;
 const trendContinuationLong=up&&hLong&&e9>e20&&s24>0&&eff24>=.12&&longConsistency>=.45,retrendContinuationShort=down&&hShort&&e9<e20&&s24<0&&eff24>=.12&&shortConsistency>=.45;
 const breakoutLong=up&&entry>rangeHigh+a*.015&&prevBar.close<=rangeHigh+a*.006&&s12>0&&barLong&&volumeHealthy,breakoutShort=down&&entry<rangeLow-a*.015&&prevBar.close>=rangeLow-a*.006&&s12<0&&barShort&&volumeHealthy;
 ...
 const compressionLong=up&&hLong&&compression&&expanding&&entry>e20&&barLong,compressionShort=down&&hShort&&compression&&expanding&&entry<e20&&barShort;
 ...
 const trendLongScore=(up?20:0)+(hLong?20:0)+(e9>e20?8:0)+(momentumLong?15:0)+((trendPullbackLong||trendContinuationLong)?15:0)+(eff24>=.16?7:0)+(longConsistency>=.50?5:0)+(rrsi>=45&&rrsi<=75?4:0)+(costAware?3:0)+(notExtended?3:0);
 const trendShortScore=(down?20:0)+(hShort?20:0)+(e9<e20?8:0)+(momentumShort?15:0)+((trendPullbackShort||retrendContinuationShort)?15:0)+(eff24>=.16?7:0)+(shortConsistency>=.50?5:0)+(rrsi>=25&&rrsi<=55?4:0)+(costAware?3:0)+(notExtended?3:0);
 const breakoutLongScore=(up?20:0)+(hLong?10:0)+(breakoutLong?20:0)+(momentumLong?15:0)+(barLong?10:0)+(volumeHealthy?8:0)+(eff24>=.12?7:0)+(costAware?5:0)+(notExtended?5:0);
 const breakoutShortScore=(down?20:0)+(hShort?10:0)+(breakoutShort?20:0)+(momentumShort?15:0)+(barShort?10:0)+(volumeHealthy?8:0)+(eff24>=.12?7:0)+(costAware?5:0)+(notExtended?5:0);
 const compressionLongScore=(up?20:0)+(hLong?20:0)+(compression?15:0)+(expanding?15:0)+(barLong?10:0)+(momentumLong?10:0)+(costAware?5:0)+(notExtended?5:0);
 const compressionShortScore=(down?20:0)+(hShort?20:0)+(compression?15:0)+(expanding?15:0)+(barShort?10:0)+(momentumShort?10:0)+(costAware?5:0)+(notExtended?5:0);
 ...
 if(trendPullbackLong||trendContinuationLong)candidates.push({side:'LONG',family:'trend',score:trendLongScore});
 if(trendPullbackShort||retrendContinuationShort)candidates.push({side:'SHORT',family:'trend',score:trendShortScore});
 if(breakoutLong)candidates.push({side:'LONG',family:'breakout',score:breakoutLongScore});
 if(breakoutShort)candidates.push({side:'SHORT',family:'breakout',score:breakoutShortScore});
 if(compressionLong)candidates.push({side:'LONG',family:'compression',score:compressionLongScore});
 if(compressionShort)candidates.push({side:'SHORT',family:'compression',score:compressionShortScore});
 if(reversionLong)candidates.push({side:'LONG',family:'reversion',score:reversionLongScore});
 if(reversionShort)candidates.push({side:'SHORT',family:'reversion',score:reversionShortScore});
 candidates.sort((a,b)=>b.score-a.score);
 const minScore=Math.max(80,cfg.minScore),winner=candidates.find(x=>x.score>=minScore),score=winner?.score??(candidates[0]?.score??0),side=winner?.side??null,family=winner?.family??'none';
 if(cfg.funnel){
  if(trendPullbackLong||trendPullbackShort||trendContinuationLong||retrendContinuationShort)cfg.funnel.familyCandidatesTrend++;
  if(breakoutLong||breakoutShort)cfg.funnel.familyCandidatesBreakout++;
  if(trendPullbackLong||trendPullbackShort)cfg.funnel.familyCandidatesRetest++;
  if(compressionLong||compressionShort)cfg.funnel.familyCandidatesCompression++;
  if(reversionLong||reversionShort)cfg.funnel.familyCandidatesReversion++;
  if(!winner){
   if(!candidates.length)cfg.funnel.noLocalPattern++;
   else if(candidates.every(x=>x.family!=='reversion'&&!((x.side==='LONG'&&momentumLong)||(x.side==='SHORT'&&momentumShort))))cfg.funnel.rejectedMomentum++;
   else if(candidates.every(x=>x.family==='trend'||x.family==='compression')&&!hLong&&!hShort)cfg.funnel.rejectedHtf++;
 ...
 const risk=Math.max(rawRisk,riskFloor),minRR=cfg.minRiskReward??1.5,maxRR=cfg.maxRiskReward??3,ultra=TRADING_CONFIG.ultraScore,scoreRR=clamp(minRR+(maxRR-minRR)*clamp((score-(cfg.minScore??TRADING_CONFIG.minScore))/Math.max(1,ultra-(cfg.minScore??TRADING_CONFIG.minScore)),0,1),minRR,maxRR),rr=family==='reversion'?Math.min(scoreRR,2):scoreRR,targetDistance=risk*rr;
 ...
 const label=family==='trend'?'Trend pullback/continuation':family==='breakout'?'Fresh breakout':family==='compression'?'Compression expansion':'Range RSI/Bollinger reversal';
 return{action:side,score:Math.round(clamp(score,0,100)),confidence:Math.round(clamp(score,0,100)),strategy:'Production Regime Breakout v30',entry,stopLoss,takeProfit,riskReward:rr,family,reasons:[label,side==='LONG'?'Bullish local regime':'Bearish local regime',family==='reversion'?'Range regime + RSI reversal':'Completed-hour confirmation + multi-horizon trend evidence','OHLC true-ATR structural stop','Cost-aware risk distance',`Target ${rr.toFixed(1)}R`,`Score ${Math.round(score)}/100`]};
}
