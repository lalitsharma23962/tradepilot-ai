...
export async function getDb(): Promise<DbClient> {
  ...
  initPromise = (async () => {
    ...
    await db.exec(`UPDATE tp_account SET max_strategies=LEAST(GREATEST(max_strategies,1),10),max_allocation_pct=LEAST(max_allocation_pct,20),leverage=LEAST(GREATEST(leverage,1),10),loss_limit_pct=LEAST(GREATEST(loss_limit_pct,0.25),20),confidence_threshold_pct=LEAST(GREATEST(confidence_threshold_pct,85),95);`);
