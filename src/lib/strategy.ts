...
/** v28: cost-aware multi-family strategy with completed-hour higher-timeframe regime. */
export function evaluateProductionStrategy(input:number[]|MarketBar[],config:Partial<StrategyConfig>={}):StrategySignal{
 const cfg={...DEFAULT_CONFIG,...config},bars=asBars(input).filter(b=>Number.isFinite(b.close)&&b.close>0).slice(-cfg.lookback),p=bars.map(b=>b.close),entry=p.at(-1)??0;if(p.length<160||!entry)return wait(entry,['Not enough history']);
 ...
 // A compression and an expansion cannot be true at the same instant. Measure
 // compression on the prior completed window, then require the current ATR ratio
 // to expand out of that compressed state. This makes the family reachable while
 // keeping the breakout trigger causal (no future data).
 ...
 const minScore=Math.max(80,cfg.minScore),side:Side|null=longFamily&&longScore>=minScore&&longScore>=shortScore?'LONG':shortFamily&&shortScore>=minScore?'SHORT':null,score=side==='LONG'?longScore:side==='SHORT'?shortScore:Math.max(longScore,shortScore);if(!side)return wait(entry,['No setup family passed the common local-regime, higher-timeframe, momentum and cost gate'],score);
 ...
 if(riskFloor>cap)return wait(entry,[`Minimum cost/ATR risk ${(riskFloor/a).toFixed(2)} ATR exceeds ${(cfg.maxStructuralRiskAtr??1.35).toFixed(2)} ATR ceiling`],score);
 ...
 const targetDistance=risk*rr,pathCapacity=a*(8+28*eff24+8*sep+8*Math.max(0,expansion-1)+5*eff48);if(targetDistance>pathCapacity)return wait(entry,[`${rr.toFixed(1)}R target exceeds measured path capacity`],score);
 const stopLoss=side==='LONG'?entry-risk:entry+risk,takeProfit=side==='LONG'?entry+targetDistance:entry-targetDistance,family=breakoutLong||breakoutShort?'Fresh breakout':retestLong||retestShort?'Breakout retest':compressionLong||compressionShort?'Compression expansion':'Trend continuation';
