...
function production(prices:number[],cfg:StrategyConfig):StrategySignal{
 ...
 const threshold=Math.max(.0001,vol*.04),longMomentum=m12>threshold,shortMomentum=m12<-threshold,sane=vol>=.0005&&vol<=.015;
 const longScore=(longTrend?30:0)+(longMomentum?20:0)+(longBreak?25:0)+(longPull?22:0)+(rrsi>=48&&rrsi<=72?10:0)+(sane?10:0);
 const shortScore=(shortTrend?30:0)+(shortMomentum?20:0)+(shortBreak?25:0)+(shortPull?22:0)+(rrsi>=28&&rrsi<=52?10:0)+(sane?10:0);
 let side:Side,score:number,reasons:string[];
 if(longScore>=shortScore&&longScore>=cfg.minScore){side='LONG';score=longScore;reasons=[longTrend?'bullish EMA regime':'',longMomentum?'positive momentum':'',longBreak?'20-bar breakout':longPull?'EMA20 pullback/reclaim':'','RSI in range','tradable volatility'].filter(Boolean);}
 else if(shortScore>longScore&&shortScore>=cfg.minScore){side='SHORT';score=shortScore;reasons=[shortTrend?'bearish EMA regime':'',shortMomentum?'negative momentum':'',shortBreak?'20-bar breakdown':shortPull?'EMA20 pullback/reclaim':'','RSI in range','tradable volatility'].filter(Boolean);}
 ...
 return{action:side,score:Math.round(clamp(score,0,100)),confidence:Math.round(clamp(score,0,100)),strategy:'Production Breakout v6',entry,stopLoss,takeProfit,riskReward,reasons};
}
