...
/** v28: cost-aware multi-family strategy with pullback-first trend entries and HTF confirmation. */
export function evaluateProductionStrategy(input:number[]|MarketBar[],config:Partial<StrategyConfig>={}):StrategySignal{
 ...
 const notExtended=Math.abs(entry-e20)<=a*2.5;
 const trendLong=up&&regimeLong&&momentumLongAligned&&eff24>=.18&&eff48>=.12&&longConsistency>=.48&&sep>=.03&&entry>=e20&&entry-e20<=a*1.25&&prevBar.close<=e20+a*.35&&notExtended;
 const trendShort=down&&regimeShort&&momentumShortAligned&&eff24>=.18&&eff48>=.12&&shortConsistency>=.48&&sep>=.03&&entry<=e20&&e20-entry<=a*1.25&&prevBar.close>=e20-a*.35&&notExtended;
 const retestLong=up&&regimeLong&&entry>e20&&prevBar.close<=e20+a*.35&&momentumLongAligned&&barLong;
 const retestShort=down&&regimeShort&&entry<e20&&prevBar.close>=e20-a*.35&&momentumShortAligned&&barShort;
 const breakoutLong=entry>rangeHigh+a*.03&&prevBar.close<=rangeHigh&&momentumLongAligned&&barLong&&breakoutVolumeLong;
 const breakoutShort=entry<rangeLow-a*.03&&prevBar.close>=rangeLow&&momentumShortAligned&&barShort&&breakoutVolumeShort;
 ...
 const costAware=vol>=Math.max(.00025,cost*.45)&&vol<=.05,longFamily=breakoutLong||retestLong||compressionLong||trendLong,shortFamily=breakoutShort||retestShort||compressionShort||trendShort;
 ...
 if(cfg.funnel){
  const patTrendLong=up&&eff24>=.18&&eff48>=.12&&longConsistency>=.48&&sep>=.03&&entry>=e20&&entry-e20<=a*1.25&&prevBar.close<=e20+a*.35&&notExtended,patTrendShort=down&&eff24>=.18&&eff48>=.12&&shortConsistency>=.48&&sep>=.03&&entry<=e20&&e20-entry<=a*1.25&&prevBar.close>=e20-a*.35&&notExtended;
  const patBreakoutLong=entry>rangeHigh+a*.03&&prevBar.close<=rangeHigh&&barLong&&breakoutVolumeLong,patBreakoutShort=entry<rangeLow-a*.03&&prevBar.close>=rangeLow&&barShort&&breakoutVolumeShort;
  const patRetestLong=up&&entry>e20&&prevBar.close<=e20+a*.35&&barLong,patRetestShort=down&&entry<e20&&prevBar.close>=e20-a*.35&&barShort;
  ...
  if(!side){if(!anyPattern)cfg.funnel.noLocalPattern++;else if(!(momentumLong||momentumShort))cfg.funnel.rejectedMomentum++;else if(!(regimeLong||regimeShort))cfg.funnel.rejectedHtf++;else cfg.funnel.rejectedScore++;}
 }
 if(!side)return wait(entry,['No setup family passed the pullback/reclaim, regime, momentum and cost gate'],score);
