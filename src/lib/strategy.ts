...
function slope(values: number[]): number {
  ...
  for (let i = 0; i < n; i++) { numerator += (i - xMean) * (values[i] - yMean); denominator += (i - xMean) ** 2; }
...
function rsi(values: number[], period = 14): number {
  ...
  for (let i = start; i < values.length; i++) { const delta = values[i] - values[i - 1]; if (delta >= 0) gain += delta; else loss -= delta; }
...
/** Strict production candidate. It never falls back to experimental profiles. */
function scoreProduction(prices: number[], cfg: StrategyConfig): StrategySignal {
  ...
  const atr = atrLike(clean, 20), volatility = atr / entry;
  ...
  const prior = clean.slice(0, -1), high20 = high(prior, 20), low20 = low(prior, 20);
...
/** Public strict production evaluator used by validation and paper execution. */
export function evaluateProductionStrategy(prices: number[], config: Partial<StrategyConfig> = {}): StrategySignal {
  return scoreProduction(prices, { ...DEFAULT_CONFIG, ...config });
}
...
function scoreProfile(prices: number[], cfg: StrategyConfig, profile: typeof PROFILES[number]): StrategySignal {
  ...
  if (clean.length < 120 || entry <= 0) return waitSignal(entry, ['Not enough history']);
  const ema20 = ema(clean, 20), ema50 = ema(clean, 50), ema100 = ema(clean, 100);
  const atr = atrLike(clean, 20), atrFast = atrLike(clean, 8), volatility = atr / entry;
  const momentum = slope(clean.slice(-12)) / entry, momentumThreshold = Math.max(volatility * 0.06, 0.00012);
  const currentRsi = rsi(clean, 14), prior = clean.slice(0, -1);
  const high12 = high(prior, 12), low12 = low(prior, 12), high20 = high(prior, 20), low20 = low(prior, 20);
  const range50 = high(prior, 50) - low(prior, 50), medium = clean.slice(-30), mediumStd = std(medium);
  const z = mediumStd > 0 ? (entry - mean(medium)) / mediumStd : 0, compressed = atr > 0 && atrFast / atr < 0.9;
  let longScore = 0, shortScore = 0; const longReasons: string[] = [], shortReasons: string[] = [];
  ...
  const longBreak = entry > high20 && entry > high12, shortBreak = entry < low20 && entry < low12;
  const longReclaim = entry > ema20 && clean.slice(-6, -1).some((p) => p <= ema20), shortReclaim = entry < ema20 && clean.slice(-6, -1).some((p) => p >= ema20);
  if (longBreak || (longReclaim && longTrend)) { longScore += profile.trigger; longReasons.push(longBreak ? '20-bar breakout' : 'EMA20 reclaim'); }
  if (shortBreak || (shortReclaim && shortTrend)) { shortScore += profile.trigger; shortReasons.push(shortBreak ? '20-bar breakdown' : 'EMA20 reclaim'); }
  ...
  const side: Side = longScore >= shortScore ? 'LONG' : 'SHORT', score = Math.max(longScore, shortScore), reasons = side === 'LONG' ? longReasons : shortReasons;
  ...
  const stopDistance = Math.max(atr * cfg.atrStopMultiple, entry * 0.0015), projection = Math.max(atr * 3.0, Math.abs(slope(clean.slice(-12))) * 42, range50 * 0.55), rawR = projection / stopDistance;
...
/** General research evaluator. It may use experimental profiles after production fails. */
export function evaluateStrategy(prices: number[], config: Partial<StrategyConfig> = {}): StrategySignal {
  ...
  if (!signals.length) return production.action === 'WAIT' ? production : waitSignal(production.entry, ['No strategy passed the complete filter set'], production.score);
