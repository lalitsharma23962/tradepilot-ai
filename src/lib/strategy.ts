...
/** v21: adaptive high-RR regime model. It broadens viable continuation/pullback
 * paths while retaining structural, volatility, cost and target-runway checks. */
function production(prices:number[],cfg:StrategyConfig):StrategySignal{
 ...
 const freshBreakoutLong=entry>hi8+a*0.05,freshBreakoutShort=entry<lo8-a*0.05;
 ...
 const costAware=vol>=Math.max(.00035,roundTripCost*.55)&&vol<=.03;
 const notExtended=Math.abs(entry-e20)<=a*1.9;
 const trendQualityLong=eff24>=.34&&eff48>=.22&&consistencyLong>=.55&&separation>=.10;
 const trendQualityShort=eff24>=.34&&eff48>=.22&&consistencyShort>=.55&&separation>=.10;
 const expansionQuality=volatilityExpansion>=.65&&volatilityExpansion<=2.2;
 const compression=dispersion(p.slice(-12))/Math.max(dispersion(p.slice(-48)),entry*0.000001)<=.78;
 const impulseLong=(entry-p[Math.max(0,p.length-7)])/Math.max(a,entry*0.000001)>=.12;
 const impulseShort=(p[Math.max(0,p.length-7)]-entry)/Math.max(a,entry*0.000001)>=.12;
 const pullImpulseLong=p.slice(-5).reduce((s,v,i,a)=>i?s+Math.max(0,v-a[i-1]):s,0)/Math.max(a,entry*0.000001)>=.10;
 const pullImpulseShort=p.slice(-5).reduce((s,v,i,a)=>i?s+Math.max(0,a[i-1]-v):s,0)/Math.max(a,entry*0.000001)>=.10;
 ...
 const longScore=(trendUp?22:mediumUp?17:0)+(e9>e20?8:0)+(momentumLong?17:0)+(breakoutSetupLong?21:pullbackSetupLong?18:0)+(rrsi>=48&&rrsi<=72?8:0)+(costAware?7:0)+(trendQualityLong?13:0)+(expansionQuality?4:0)+(notExtended?4:0)+(compression?2:0);
 const shortScore=(trendDown?22:mediumDown?17:0)+(e9<e20?8:0)+(momentumShort?17:0)+(breakoutSetupShort?21:pullbackSetupShort?18:0)+(rrsi>=28&&rrsi<=52?8:0)+(costAware?7:0)+(trendQualityShort?13:0)+(expansionQuality?4:0)+(notExtended?4:0)+(compression?2:0);
