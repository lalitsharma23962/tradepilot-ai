...
const DEFAULT_CONFIG:StrategyConfig={
 minScore:55,minRiskReward:1.5,maxRiskReward:2.0,atrStopMultiple:1.25,
...
// from close-to-close movement. The historical validator uses the signal's own
// stop/target so paper and validation share exactly the same decision logic.
...
function production(prices:number[],cfg:StrategyConfig):StrategySignal{
 ...
 const breakoutLong=entry>h24*(1+0.00005),breakoutShort=entry<l24*(1-0.00005);
 const pullLong=(trendUp||mediumUp)&&entry>e20&&m12>0&&p.slice(-6,-1).some(v=>v<=e20*1.001);
 const pullShort=(trendDown||mediumDown)&&entry<e20&&m12<0&&p.slice(-6,-1).some(v=>v>=e20*0.999);
 const momentumThreshold=Math.max(0.00002,vol*0.0125);
 ...
 const notExtended=Math.abs(entry-e20)<=a*2.8;
 const longScore=(trendUp?25:mediumUp?18:0)+(fastUp?10:0)+(momentumLong?22:0)+(breakoutLong?15:0)+(pullLong?15:0)+(rrsi>=45&&rrsi<=72?10:0)+(costAwareVol?8:0);
 const shortScore=(trendDown?25:mediumDown?18:0)+(fastDown?10:0)+(momentumShort?22:0)+(breakoutShort?15:0)+(pullShort?15:0)+(rrsi>=28&&rrsi<=55?10:0)+(costAwareVol?8:0);
 let side:Side,score:number,reasons:string[];
 const longSetup=(trendUp||mediumUp)&&(momentumLong||breakoutLong||pullLong||fastUp)&&rrsi>=45&&rrsi<=72&&costAwareVol&&notExtended;
 const shortSetup=(trendDown||mediumDown)&&(momentumShort||breakoutShort||pullShort||fastDown)&&rrsi>=28&&rrsi<=55&&costAwareVol&&notExtended;
 if(longSetup&&longScore>=cfg.minScore){
  side='LONG';score=longScore;reasons=[trendUp?'bullish EMA regime':mediumUp?'positive EMA/momentum regime':'',momentumLong?'positive multi-horizon momentum':'',breakoutLong?'24-bar breakout':pullLong?'EMA20 pullback/reclaim':fastUp?'EMA9/20 momentum':'','RSI confirmation','cost-aware volatility','price not excessively extended'].filter(Boolean);
 } else if(shortSetup&&shortScore>=cfg.minScore){
  side='SHORT';score=shortScore;reasons=[trendDown?'bearish EMA regime':mediumDown?'negative EMA/momentum regime':'',momentumShort?'negative multi-horizon momentum':'',breakoutShort?'24-bar breakdown':pullShort?'EMA20 pullback/reclaim':fastDown?'EMA9/20 momentum':'','RSI confirmation','cost-aware volatility','price not excessively extended'].filter(Boolean);
 } else return wait(entry,['Production setup below quality threshold'],Math.max(longScore,shortScore));
 const dist=Math.max(a*cfg.atrStopMultiple,entry*.0012),riskReward=clamp(1.7,cfg.minRiskReward,cfg.maxRiskReward),stopLoss=side==='LONG'?entry-dist:entry+dist,takeProfit=side==='LONG'?entry+dist*riskReward:entry-dist*riskReward;
 return{action:side,score:Math.round(clamp(score,0,100)),confidence:Math.round(clamp(score,0,100)),strategy:'Production Breakout v12',entry,stopLoss,takeProfit,riskReward,reasons};
}
...
export function evaluateResearchStrategy(prices:number[],config:Partial<StrategyConfig>={}):StrategySignal{return production(prices,{...DEFAULT_CONFIG,...config,minScore:config.minScore??55});}
