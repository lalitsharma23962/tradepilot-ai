...
const DEFAULT_CONFIG:StrategyConfig={minScore:70,minRiskReward:1.5,maxRiskReward:2.2,atrStopMultiple:1.25,lookback:240,strategyLimit:17,feeBps:10,slippageBps:2};
...
function production(prices:number[],cfg:StrategyConfig):StrategySignal{
 ...
 const e9=ema(p,9),e20=ema(p,20),e50=ema(p,50),e100=ema(p,100),a=atr(p),vol=a/entry,rrsi=rsi(p),s12=slope(p.slice(-12))/entry,s36=slope(p.slice(-36))/entry,prior=p.slice(0,-1);
 const hi20=Math.max(...prior.slice(-20)),lo20=Math.min(...prior.slice(-20)),trendUp=e20>e50&&e50>e100,trendDown=e20<e50&&e50<e100,mediumUp=e20>e50&&s36>0,mediumDown=e20<e50&&s36<0;
 ...
 const roundTripCost=(2*(cfg.feeBps??10)+2*(cfg.slippageBps??2))/10000,costAware=vol>=Math.max(.00045,roundTripCost*.55)&&vol<=.025,notExtended=Math.abs(entry-e20)<=a*2.8;
