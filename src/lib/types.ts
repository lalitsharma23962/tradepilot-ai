...
export interface Performance{equity:number;total_pnl:number;realized_pnl:number;unrealized_pnl:number;win_rate:number;wins:number;losses:number;trade_count:number;open_positions:number;best_trade:number;worst_trade:number;avg_trade:number;profit_factor:number;max_drawdown:number;}
