...
export interface Position{id:string;symbol:string;side:Side;quantity:number;entry_price:number;current_price:number;notional:number;unrealized_pnl:number;stop_loss:number;take_profit:number;strategy:string;status:string;opened_at:string;initial_risk?:number;target_stage?:number;timeframe?:string;}
...
export interface Snapshot{equity:number;cash:number;open_value:number;unrealized_pnl:number;realized_pnl:number;ts:string;}
...
export interface AiTarget{r:number;fraction:number;price:number;}
export interface AiRecommendation{symbol:string;action:'LONG'|'SHORT'|'WAIT';confidence:number;threshold?:number;entry:number;stop_loss:number;take_profit:number;risk_score:number;explanation:string;targets?:AiTarget[];}
