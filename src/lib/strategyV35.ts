...
export function evaluateProductionStrategy(input:number[]|MarketBar[],config:Partial<StrategyConfig>={}):StrategySignal{
 ...
 const familyScore=(family:string,side:Side)=>{const long=side==='LONG',dir=long?longDirectional:shortDirectional,mom=long?hLong:hShort,cons=long?longCons:shortCons,bar=long?body>.25&&closeLoc>=.40:body>.25&&closeLoc<=.60,vol_=long?volumeHealthy:volumeHealthy;let s=dir*8;if(mom)s+=12;if(cons>=.60)s+=8;if(bar)s+=5;if(vol_)s+=3;if(family==='trend'&&(long?up:down))s+=10;if(family==='breakout'&&compression&&expanding)s+=12;if(family==='compression'&&compression)s+=8;return Math.max(0,s);};
 ...
 const winner=candidates.sort((a,b)=>b.score-a.score)[0];if(!winner)return wait(entry,['No candidate family qualified'],0);const score=winner.score;
 ...
 const rawRisk=side==='LONG'?Math.max(entry-swingLow,a*cfg.atrStopMultiple??TRADING_CONFIG.atrStopMultiple):Math.max(swingHigh-entry,a*(cfg.atrStopMultiple??TRADING_CONFIG.atrStopMultiple)),minRisk=a*(cfg.minStopAtr??TRADING_CONFIG.minStopAtr),maxRisk=a*(cfg.maxStructuralRiskAtr??TRADING_CONFIG.maxStructuralRiskAtr);
 if(rawRisk<minRisk){if(cfg.funnel)cfg.funnel.rejectedRiskFloor++;return wait(entry,['Structural risk is below the minimum ATR floor'],score);}if(rawRisk>maxRisk){if(cfg.funnel)cfg.funnel.rejectedRiskCeiling++;return wait(entry,['Structural risk exceeds the maximum ATR ceiling'],score);}
 ...
 const targetDistance=risk*chosenR,takeProfit=side==='LONG'?entry+targetDistance:entry-targetDistance,quality=score>=TRADING_CONFIG.ultraScore?'ultra-conviction':'qualified';
 return{action:side,score:Math.round(clamp(score,0,100)),confidence:Math.round(clamp(score,0,100)),strategy:'Production Regime Breakout v38',entry,stopLoss:stopPrice,takeProfit,riskReward:2,family,quality,...(cfg.funnel?{funnel:cfg.funnel}:{})};
}
