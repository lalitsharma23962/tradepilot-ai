...
function signal(id:string,c:Candle[],cfg:BacktestConfig):1|-1|0{
 ...
 const volOk=v>=tv*.7&&v<=tv*3.5,buffer=Math.max(tv*.15,.0004)*x,confirm=vr>=.9,notExt=Math.abs(x-e20)<=a*2.4;
 switch(id){
...
export async function runValidation(symbol='BTCUSDT',interval='1h',cfg:Partial<BacktestConfig>={},selectedStrategyId?:string,preloadedCandles?:Candle[],preOosOnly=false):Promise<ValidationReport>{
 const config={...DEFAULT_BACKTEST_CONFIG,...cfg},candles=preloadedCandles??await fetchHistoricalCandles(symbol,interval,MAX_HISTORY_BARS);if(candles.length<MIN_HISTORY)throw Error(`Need ${MIN_HISTORY} completed candles; received ${candles.length}.`);
 ...
 const pick=selectedStrategyId&&eligible.some(x=>x.s.id===selectedStrategyId)?eligible.find(x=>x.s.id===selectedStrategyId):eligible[0];const test=pick&&!preOosOnly?simulate(candles,pick.s.id,config,pre,n):null,mc=test?monte(test.tradeReturnsPct):monte([]),reasons:string[]=[];
