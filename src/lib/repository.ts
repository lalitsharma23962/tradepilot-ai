...
function normalizeAccount(r: Record<string, unknown>): Account {
  return {
    ...
    max_positions: num(r.max_positions) || 3,
    max_allocation_pct: num(r.max_allocation_pct) || 20,
...
export async function getPositions(): Promise<Position[]> {
  const rows = await query<Record<string, unknown>>(
    `SELECT * FROM tp_positions WHERE status = 'OPEN' ORDER BY opened_at DESC;`
  );
...
export async function getTrades(limit = 200): Promise<Trade[]> {
  const rows = await query<Record<string, unknown>>(
    `SELECT * FROM tp_trades ORDER BY closed_at DESC LIMIT $1;`,
    [limit]
  );
...
export async function getSnapshots(limit = 500): Promise<Snapshot[]> {
  const rows = await query<Record<string, unknown>>(
    `SELECT * FROM tp_snapshots ORDER BY ts DESC LIMIT $1;`,
    [limit]
  );
  return rows
...
export async function getPerformance(): Promise<Performance> {
  ...
  let maxEquity = account.equity;
  let maxDrawdown = 0;
  for (const s of snapshots) {
    if (s.equity > maxEquity) maxEquity = s.equity;
    const dd = maxEquity - s.equity;
...
export async function getSettings(): Promise<Settings> {
  ...
  return {
    ...
    max_allocation_pct: account.max_allocation_pct,
    ...
    risk_alerts: account.risk_alerts,
  };
}
...
export async function updateSettings(s: Partial<Settings>): Promise<Settings> {
  ...
  if (s.max_allocation_pct !== undefined) {
    const v = Math.min(20, Math.max(5, s.max_allocation_pct));
    push('max_allocation_pct', v);
  }
  if (s.default_allocation_pct !== undefined) {
    const v = Math.min(20, Math.max(5, s.default_allocation_pct));
    push('default_allocation_pct', v);
  }
  ...
  if (set.length > 0) {
    params.push(1);
    await execute(`UPDATE tp_account SET ${set.join(', ')} WHERE id = $${params.length};`, params);
  }
  return getSettings();
}