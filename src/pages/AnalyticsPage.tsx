...
export function AnalyticsPage() {
  ...
  const runValidation = async () => {
    ...
    try {
      ...
      if(res.ok) {
        ...
        if(res.data?.gate.status==='REJECTED') {
          setSelectedStrategyId(undefined);
        } else if(res.data?.walkForward.selectedStrategy) {
  ...
  return <div className="space-y-6 p-6">
    ...
    <Card><CardHeader title="Historical Validation Lab" subtitle="Up to 40,000 Binance candles → fees + slippage → 50/20/30 walk-forward → two validation folds → untouched OOS → Monte Carlo" action={<Button onClick={runValidation} disabled={running}>{running?'Running…':'Run validation'}</Button>}/><CardBody className="space-y-4">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5"><div className="rounded-lg bg-slate-800/40 p-3"><p className="text-[11px] text-slate-500">Data</p><p className="text-sm font-semibold">BTCUSDT · 5m</p></div><div className="rounded-lg bg-slate-800/40 p-3"><p className="text-[11px] text-slate-500">Selected strategy</p><div className="mt-1 flex gap-2"><select value={selectedStrategyId ?? ''} onChange={e=>setSelectedStrategyId(e.target.value||undefined)} disabled={running} className="min-w-0 flex-1 rounded border border-slate-700 bg-slate-900 px-2 py-1 text-sm font-semibold text-slate-200"><option value="">Walk-forward auto-select</option>{STRATEGIES.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select>{selectedStrategyId&&<Button size="sm" variant="secondary" onClick={()=>setSelectedStrategyId(undefined)} disabled={running}>Auto</Button>}</div></div><div className="rounded-lg bg-slate-800/40 p-3"><p className="text-[11px] text-slate-500">Fee</p><p className="text-sm font-semibold">{Number(account?.fee_bps??10)} bps/side</p></div><div className="rounded-lg bg-slate-800/40 p-3"><p className="text-[11px] text-slate-500">Slippage</p><p className="text-sm font-semibold">{Number(account?.slippage_bps??2)} bps/side</p></div><div className="rounded-lg bg-slate-800/40 p-3"><p className="text-[11px] text-slate-500">Leverage</p><p className="text-sm font-semibold">≤{displayedLeverage}x</p></div></div>
      {selectedStrategyId&&<p className="text-xs text-slate-400">Manual selection is a research override only. Auto-select is recommended and uses pre-OOS evidence; the final OOS window is never used to choose the strategy.</p>}
