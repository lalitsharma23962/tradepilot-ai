...
// 5m is included for the user's higher-frequency research requirement. It does not
// weaken the strict gate: only a fully validated candidate can unlock paper trading.
const VALIDATION_INTERVALS = ['5m','15m','1h','4h'] as const;
type ValidationInterval = typeof VALIDATION_INTERVALS[number];
...
export function AnalyticsPage() {
  ...
  const [interval, setInterval] = useState<ValidationInterval>('15m');
  ...
  const runValidation = async () => {
    ...
    try {
      ...
      const res=await api.validationApi('BTCUSDT',interval,{initialCapital:10000,feeBps:Number(account?.fee_bps??10),slippageBps:Number(account?.slippage_bps??2),maxPositionPct,leverage,riskPerTradePct});
  ...
  return <div className="space-y-6 p-6">
    ...
    <Card><CardHeader title="Historical Validation Lab" subtitle="20,000 Binance candles → fees + slippage → strict 10R/15R production targets → three non-overlapping pre-OOS stability folds → untouched 30% OOS → Monte Carlo" action={<div className="flex items-center gap-2"><label className="text-xs text-slate-400">Timeframe</label><select value={interval} onChange={e=>setInterval(e.target.value as ValidationInterval)} disabled={running} className="rounded-md border border-slate-700 bg-slate-900 px-2 py-1.5 text-xs text-slate-200"><option value="5m">5m</option><option value="15m">15m</option><option value="1h">1h</option><option value="4h">4h</option></select><Button onClick={runValidation} disabled={running}>{running?`Validating… ${elapsed}s`:'Run validation'}</Button></div>}/><CardBody className="space-y-4">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4"><div className="rounded-lg bg-slate-800/40 p-3"><p className="text-[11px] text-slate-500">Data</p><p className="text-sm font-semibold">BTCUSDT · {interval} · 20,000 candles</p></div><div className="rounded-lg bg-slate-800/40 p-3"><p className="text-[11px] text-slate-500">Strategy selection</p><p className="text-sm font-semibold text-sky-300">Strict high-RR production strategy</p><p className="mt-1 text-[11px] text-slate-500">10R targets for qualified signals · 15R for ultra-high score signals</p></div><div className="rounded-lg bg-slate-800/40 p-3"><p className="text-[11px] text-slate-500">Fee</p><p className="text-sm font-semibold">{Number(account?.fee_bps??10)} bps/side</p></div><div className="rounded-lg bg-slate-800/40 p-3"><p className="text-[11px] text-slate-500">Slippage</p><p className="text-sm font-semibold">{Number(account?.slippage_bps??2)} bps/side</p></div></div>
      <div className="rounded-lg border border-sky-900/50 bg-sky-950/20 px-4 py-3 text-xs text-slate-300">The validator selects a candidate only after it passes all three non-overlapping pre-OOS stability folds. A strategy must be profitable with PF ≥ 1.05 and sufficient trade count in every fold before it is eligible for automatic OOS selection. The final 30% of history remains completely untouched and is used only for the final gate. Use 5m/15m for higher signal frequency; 1h/4h are available for slower confirmation regimes.</div>
      {running&&<div className="rounded-lg border border-sky-800/60 bg-sky-950/30 px-4 py-3 text-xs text-sky-200">Validation is running the strict production 10R/15R pre-OOS test plus the research comparison scan. Elapsed time: <strong>{elapsed}s</strong>. Keep this tab open; do not start the paper bot while validation is running.</div>}
      ...
      {report&&<>
        ...
        <div><p className="mb-2 text-xs text-slate-500">Pre-OOS strategy candidates — ranked for research visibility. The validator chooses automatically only from candidates that pass the stability filter; this table cannot be used to tune against the untouched OOS window.</p><div className="overflow-x-auto"><table className="w-full text-left text-xs"><thead className="text-slate-500"><tr><th className="px-2 py-2">Strategy</th><th>Trades</th><th>Win %</th><th>PF</th><th>Return</th><th>Max DD</th><th>Score</th></tr></thead><tbody>{report.strategies.map(s=><tr key={s.id} className={`border-t border-slate-800 ${report.walkForward.selectedStrategy===s.name?'bg-sky-950/40':''}`}><td className="px-2 py-2 font-medium text-slate-200">{s.name}{report.walkForward.selectedStrategy===s.name&&<span className="ml-2 text-[10px] text-sky-300">AUTO-SELECTED</span>}</td><td>{s.trades}</td><td>{s.winRate.toFixed(1)}%</td><td>{s.profitFactor.toFixed(2)}</td><td className={s.returnPct>=0?'text-emerald-400':'text-red-400'}>{s.returnPct.toFixed(2)}%</td><td className="text-red-300">{s.maxDrawdownPct.toFixed(2)}%</td><td>{s.score.toFixed(2)}</td></tr>)}</tbody></table></div></div>
        <div className="grid grid-cols-1 gap-3 md:grid-cols-4"><div className="rounded-lg border border-slate-800 p-3"><p className="text-[11px] text-slate-500">Walk-forward selected</p><p className="mt-1 text-sm font-semibold">{report.walkForward.selectedStrategy}</p><p className="mt-1 text-xs text-slate-500">Pre-OOS folds {report.walkForward.trainBars} / {report.walkForward.validationBars} / {report.walkForward.testBars} bars</p></div><div className="rounded-lg border border-slate-800 p-3"><p className="text-[11px] text-slate-500">Validation result</p><p className="mt-1 text-sm font-semibold">{report.walkForward.validation ? `${report.walkForward.validation.returnPct.toFixed(2)}% return · PF ${report.walkForward.validation.profitFactor.toFixed(2)} · ${report.walkForward.validation.trades} trades` : 'No eligible validation candidate'}</p></div><div className="rounded-lg border border-slate-800 p-3"><p className="text-[11px] text-slate-500">Out-of-sample test</p><p className="mt-1 text-sm font-semibold">{report.walkForward.test ? `${report.walkForward.test.returnPct.toFixed(2)}% return · PF ${report.walkForward.test.profitFactor.toFixed(2)} · ${report.walkForward.test.trades} trades` : 'Not run because no candidate passed pre-OOS stability'}</p></div><div className="rounded-lg border border-slate-800 p-3"><p className="text-[11px] text-slate-500">Monte Carlo</p><p className="mt-1 text-sm font-semibold">{report.monteCarlo.probabilityOfLoss.toFixed(1)}% loss probability</p><p className="mt-1 text-xs text-slate-500">Median {report.monteCarlo.medianReturnPct.toFixed(2)}% · P05 {report.monteCarlo.p05ReturnPct.toFixed(2)}% · DD P95 {report.monteCarlo.p95MaxDrawdownPct.toFixed(2)}%</p></div></div>
      </>}
