...
export function AnalyticsPage() {
    ...
    try {
      ...
      if(res.ok) {
        ...
        if(res.data?.gate.status==='REJECTED') {
          // A failed OOS run must never become a sticky manual override. Clear it so the next run re-selects from pre-OOS evidence only.
  ...
  return <div className="space-y-6 p-6">
    ...
    <Card><CardHeader title="Historical Validation Lab" subtitle="20,000 real Binance candles → fees + slippage → train/validation/test walk-forward → Monte Carlo" action={<Button onClick={runValidation} disabled={running}>{running?'Running…':'Run validation'}</Button>}/><CardBody className="space-y-4">
      ...
      {selectedStrategyId&&<p className="text-xs text-slate-400">Manual selection is a research override only. Auto-select is recommended and uses pre-OOS evidence; the final 50% OOS window is never used to choose the strategy.</p>}
