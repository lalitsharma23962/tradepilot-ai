...
export function SettingsPage() {
  ...
  return <div className="mx-auto max-w-3xl space-y-6 p-6">
    ...
    <Card><CardHeader title="Minimum Confidence / Score Threshold" subtitle="85–95. The 85% floor matches the historical validation gate." /><CardBody className="space-y-3"><div className="flex items-center gap-4"><input type="range" min={85} max={95} step={1} value={settings.confidence_threshold_pct} onChange={e=>update({confidence_threshold_pct:Number(e.target.value)})} disabled={saving} className="flex-1 accent-sky-500"/><span className="w-16 text-right text-sm font-semibold text-sky-400">{settings.confidence_threshold_pct}%</span></div><p className="text-xs text-slate-500">Applied to the paper engine on the next tick. It cannot weaken the hard 85% validated floor or other risk limits.</p></CardBody></Card>
