...
/** Build only fully completed hourly buckets from lower-timeframe OHLCV.
 * This is deliberately derived from the already-available historical bars, so it
 * cannot introduce future information into a lower-timeframe decision. */
function completedHourly(bars:MarketBar[]):MarketBar[]{
 if(bars.length<8)return[];
 const steps=bars.slice(1).map((b,i)=>b.openTime-bars[i].openTime).filter(x=>x>0).sort((a,b)=>a-b);
 ...
 for(const b of bars){const key=Math.floor(b.openTime/3600000)*3600000;const g=groups.get(key);if(g)g.push(b);else groups.set(key,[b]);}
 return Array.from(groups.entries()).sort((a,b)=>a[0]-b[0]).filter(([,g])=>g.length===perHour).map(([openTime,g])=>({
  openTime,open:g[0].open,high:Math.max(...g.map(x=>x.high)),low:Math.min(...g.map(x=>x.low)),close:g[g.length-1].close,volume:g.reduce((s,x)=>s+x.volume,0)
 }));
}

/** v28: cost-aware multi-family strategy with a completed-hour higher-timeframe regime.
 * The research baseline is intentionally 1.5R-3R. The strict validation gate remains
 * unchanged; this version is not allowed to pass by lowering the gate. */
export function evaluateProductionStrategy(input:number[]|MarketBar[],config:Partial<StrategyConfig>={}):StrategySignal{
 ...
 const hourly=completedHourly(bars),hp=hourly.map(b=>b.close),h20=ema(hp,20),h40=ema(hp,40),h50=ema(hp,50);
 ...
 const hLong=hourly.length>=50?h20>h40&&h40>h50&&hS12>0&&hS24>=0&&hEff24>=.12:true;
 const hShort=hourly.length>=50?h20<h40&&h40<h50&&hS12<0&&hS24<=0&&hEff24>=.12:true;
 const momentumLongAligned=momentumLong&&hLong,momentumShortAligned=momentumShort&&hShort;
 const trendLong=up&&hLong&&eff24>=.20&&eff48>=.15&&longConsistency>=.50&&sep>=.04;
 const trendShort=down&&hShort&&eff24>=.20&&eff48>=.15&&shortConsistency>=.50&&sep>=.04;
 ...
 const risk=Math.max(rawRisk,a*(cfg.atrStopMultiple??1.5)*.65,entry*cost*1.75),rr=clamp(cfg.riskReward??(score>=TRADING_CONFIG.ultraScore?cfg.maxRiskReward??3:cfg.minRiskReward??1.5),cfg.minRiskReward??1.5,cfg.maxRiskReward??3),targetDistance=risk*rr;
 ...
 const family=breakoutLong||breakoutShort?'Fresh breakout':retestLong||retestShort?'Breakout retest':'Trend/compression continuation';
 return{action:side,score:Math.round(clamp(score,0,100)),confidence:Math.round(clamp(score,0,100)),strategy:'Production Regime Breakout v28',entry,stopLoss,takeProfit,riskReward:rr,reasons:[family,side==='LONG'?'Bullish local + hourly regime':'Bearish local + hourly regime','Multi-horizon momentum','Completed-hour confirmation','OHLC true-ATR structural stop','Cost-aware risk distance',`Target ${rr.toFixed(1)}R`,`Score ${Math.round(score)}/100`]};
}
